...
*Last updated: 2026-04-24 (D6 v2 pivot: Patch-A REVERTED after empirical diagnostic proved the client-side travel.iff + planet_crc_string_table.iff gate travel-UI visibility regardless of server-side Zone::getClientZoneName rewrites. extraction_outpost pivots to a first-class 11th client-known planet via `extraction_outpost_01.tre` mod TRE. See `workplans/phase-1/deliverable-06-travel-hook.md` v2 for the discovery trail and new session plan.) Previously: Phase 1 Deliverable 5 close-out — Patch-C + Patch-G landed: death corpse-drop spawns bag contents at death coords for any-player loot, drag auto-route redirects tagged items back into the Extraction Pack; the PlaceableLootContainerComponent swap on the corpse template was the load-bearing architectural fix that unlocked drag-out. Phase 1 Deliverable 4b close-out — Patch-F landed: loot auto-route into extraction bag on-zone via lootAll. Phase 1 Deliverable 1 close-out — Patch-A landed, verified in-client. Each subsequent patch landing appends a commit + updates this table's Status column.*
